...
resource "azuredevops_build_definition" "build" {
  project_id = azuredevops_project.project.id
  name       = "AgileSample Build Definition"
  path       = "\\ExampleFolder"
...
resource "azuredevops_serviceendpoint_kubernetes" "kubeendpoint1" {
  project_id            = azuredevops_project.project.id
  service_endpoint_name = "AgileSample Kubernetes"
  ...
  azure_subscription {
    ...
    resourcegroup_id  = "AgileSample-rg"
    namespace         = "default"
    cluster_name      = "AgileSample-aks"
  }
}
...
resource "azuredevops_serviceendpoint_kubernetes" "kubeendpoint2" {
  project_id            = azuredevops_project.project.id
  service_endpoint_name = "AgileSample Kubernetes"
  apiserver_url         = "https://agilesample-aks.hcp.eastus.azmk8s.io"
...
resource "azuredevops_serviceendpoint_kubernetes" "serviceendpoint" {
  project_id            = azuredevops_project.project.id
  service_endpoint_name = "AgileSample Kubernetes"
  apiserver_url         = "https://agilesample-kubernetes-cluster.hcp.eastus.azmk8s.io"
